...
const DeliveryPage = () => {
  ...
  const allowedStatuses = ["PICKUP EXCEPTION", "OUT FOR PICKUP", "PICKUP SCHEDULED"];
  ...
  const processScannedData = (scannedValue) => {
    ...
    if (!orderDetails || deliveryStage === "COMPLETED") {
      // Reset state if delivery is completed to allow new AWB scan
      if (deliveryStage === "COMPLETED") {
        setOrderDetails(null);
        setAwbCode("");
        setFullAwbCode("");
        setStatusMessage("Ready to scan AWB Code");
    ...
    axios
      ...
      .then((res) => {
        if (!res.data.order) {
          ...
          if (res.data.order.packed_status !== "Completed") {
  ...
  const updateDeliveryStatus = () => {
    ...
    axios
      .post(`${config.apiBaseUrl}/order/awb/${encodeURIComponent(awbCode)}/delivery`, {
        warehouse_out: "Yes",
